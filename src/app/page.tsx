...
async function getStats(fetchWithProfile: (url: string) => Promise<Response>): Promise<Stats> {
  const [pending, scheduled, approved, posted, archived, discarded] = await Promise.all([
  ...
  return {
    ...
    // Count both archived and discarded
    archived: (archived.events?.length || 0) + (discarded.events?.length || 0),
  }
}
